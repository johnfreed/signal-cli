...
linkAndDisplay(command<s>) -> <>::
linkAndDisplay(command<s>, newDeviceName<s>) -> <>::
...
Exceptions: None; returns empty array if group is not found. Use isGroupFound to test.
...
* groupId       : Byte array representing the internal group identifier
* base64GroupId : String representing the internal group identifier in Base64 format
* useExceptions : true = raise exceptions; false = do not raise exceptions
...
Exceptions: None; returns empty string if group is not found. Use isGroupFound to test.
...
Exceptions: None; returns false if group is not found. Use isGroupFound to test.
...
isGroupFound(groupId<ay>) -> isFound<b>::
* groupId  : Byte array representing the internal group identifier
* isFound  : true = group found; false = group not found
