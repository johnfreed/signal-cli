...
public class DbusSignalControlImpl implements org.asamk.SignalControl {
    ...
    public DbusSignalControlImpl(
            final SignalCreator c, final Function<Manager, Thread> newManagerRunner
    ) {
        this.c = c;
    ...
    public List<String> listAccounts() {
        synchronized (receiveThreads) {
            return receiveThreads.stream().map(Pair::first).map(Manager::getUsername).collect(Collectors.toList());
        }
    }
}
